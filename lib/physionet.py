...
def patch_variable_time_collate_fn(
    ...
    if args.dataset != "ushcn":
        combined_vals = utils.normalize_masked_data(
            combined_vals,
            combined_mask,
            att_min=data_min.to(device),
            att_max=data_max.to(device),
        )
...
def variable_time_collate_fn(
    ...
    Expects a batch of time series data in the form of (record_id, tt, vals, mask) where
            ...
            - vals is a (T, D) tensor containing observed values for D variables.
            - mask is a (T, D) tensor containing 1 where values were observed and 0 otherwise.
            - sparsity: what share of the data should be keept.
    Returns:
    ...
    # random_mask_obs = (
    #    torch.rand(observed_mask.shape, device=observed_mask.device) <= sparsity
    # )
    # random_mask_target = (
    #    torch.rand(predicted_mask.shape, device=predicted_mask.device) <= sparsity
    # )
    #
    # observed_mask = observed_mask * random_mask_obs
    # predicted_mask = predicted_mask * random_mask_target
    #
    # observed_data = observed_data * random_mask_obs
    # predicted_data = predicted_data * random_mask_target
    #
    ## Remove entries where observed_mask is false everywhere across dimension -1
    # mask_sum = observed_mask.sum(dim=-1) > 0
    # observed_tp = observed_tp[mask_sum]
    # observed_data = observed_data[mask_sum]
    # observed_mask = observed_mask[mask_sum]

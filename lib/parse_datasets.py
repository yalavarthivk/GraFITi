...
def parse_datasets(args, patch_ts=False, length_stat=False):
    ...
    if dataset_name in ["physionet", "mimic"]:
        ...
        # else:
        # 	device_ = device
        data_min, data_max, time_max = get_data_min_max(
            seen_data, device
        )  # (n_dim,), (n_dim,)
        ...
        if length_stat:
            max_input_len, max_pred_len, median_len = get_seq_length(
                args, total_dataset
            )
            ...
            print(
                data_objects["max_input_len"],
                data_objects["max_pred_len"],
                data_objects["median_len"],
            )
    ...
    ### USHCN dataset ###
        ...
        if length_stat:
            max_input_len, max_pred_len, median_len = USHCN_get_seq_length(
    ...
    ### Activity dataset ###
        ...
        total_dataset = PersonActivity(
            "../data/activity/", n_samples=args.n, download=True, device=device
        )
        ...
        batch_size = min(min(len(seen_data), args.batch_size), args.n)
        data_min, data_max, _ = get_data_min_max(
            seen_data, device
        )  # (n_dim,), (n_dim,)
        ...
        if length_stat:
            max_input_len, max_pred_len, median_len = Activity_get_seq_length(
                args, train_data + val_data + test_data
            )
            data_objects["max_input_len"] = max_input_len.item()
            data_objects["max_pred_len"] = max_pred_len.item()

...
def tsdm_collate(batch):
    ...
    times = torch.cat(context_x, dim = 0)
    ...
    # sort = torch.sort(times)
    sort_ids = torch.argsort(times)

    times_, time_ptr = times.unique(return_counts=True)
    time_ptr = torch.cat([torch.tensor([0.]).to(time_ptr.device), time_ptr], dim=0).to(torch.int64)
    values_ = values[sort_ids]
    mask_ = mask[sort_ids]
    pat_idx = idx[sort_ids]

    res["pat_idx"]  = pat_idx.numpy()
    res["times"]    = times_.numpy()
    res["time_ptr"] = time_ptr.cumsum(0).numpy()
    res["X"]        = values_
    res["M"]        = mask_
    res["obs_idx"]  = pat_idx.to(torch.int64).numpy()
...
def tsdm_collate_val(batch):
    ...
    for i, sample in enumerate(batch):
        ...
        y_vals.append(y)
        y_mask.append(mask_y)
        idx_.append(torch.ones_like(t)*i)
        idx_val_.append(torch.ones_like(t_target)*i)
        context_x.append(t)
    ...
    times_val = torch.cat(target_x, dim = 0)
    idx_val = torch.cat(idx_val_, dim = 0)
    values_val = torch.cat(target_vals, dim=0)
    mask_val = torch.cat(target_mask, dim=0)

    # sort = torch.sort(times)
    # sort_ids_val = torch.argsort(times_val)

    # times_val_, time_ptr_val = times_val.unique(return_counts=True)
    # time_ptr_val = torch.cat([torch.tensor([0.]).to(time_ptr_val.device), time_ptr_val], dim=0).to(torch.int64)
    # values_val_ = values_val[sort_ids_val]
    # mask_val_ = mask_val[sort_ids_val]
    # pat_idx_val = idx_val[sort_ids_val]


    res = {}
    res["pat_idx"]  = pat_idx.numpy()
    res["times"]    = times_.numpy()
    res["time_ptr"] = time_ptr.cumsum(0)
    res["X"]        = values_
    res["M"]        = mask_
    res["obs_idx"]  = pat_idx.to(torch.int64).numpy()
    res["y"]        = torch.zeros([i+1,1])
    ...
    res["M_val"]    = mask_val
    res["times_val"]= times_val.numpy()
    res["index_val"]= idx_val.numpy()
